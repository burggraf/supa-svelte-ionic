if [ x"${PROD_DB}" == "x" ]; then 
     echo "You need to set PROD_DB to the URL of the database you want to backup."
else
pg_dump \
  --clean \
  --if-exists \
...
| gzip -c > ~/dev/supa-svelte-ionic-backup/prod/supa-svelte-ionic-backup-`date "+%Y.%m.%d-%H.%M.%S"`.sql.gz
